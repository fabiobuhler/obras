...
const exameSchema = z.object({
  tipo_exame: z.string().min(2, 'O tipo de exame é obrigatório'),
...
export default function ExameForm({ initialData, funcionarioId, onSubmit, onCancel }) {
  ...
  const { register, handleSubmit, formState: { errors }, reset, setValue, watch } = useForm({
    ...
    defaultValues: initialData || { resultado: 'Apto' }
  });
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            {...register('tipo_exame')}
            className={`w-full rounded-md border bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary ${errors.tipo_exame ? 'border-red-500' : 'border-gray-300 dark:border-gray-700'}`}
            placeholder="Ex: Admissional, Periódico, Demissional"
          />
          {errors.tipo_exame && <span className="text-red-500 text-xs">{errors.tipo_exame.message}</span>}
        </div>
        ...
        <div className="col-span-2">
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium">Anexar Exame</label>
          ...
          {uploadMode === 'link' ? (
            <input
              ...
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              placeholder="https://meu-drive.com/exame.pdf"
            />
          ) : (
                ...
                <>
                  ...
                  {arquivoUrlAtual && !selectedFile && (
                    <div className="absolute inset-0 bg-white dark:bg-zinc-900 flex flex-col items-center justify-center rounded-md z-10 p-4 border-2 border-green-500/50">
                      ...
                      <p className="text-sm text-center mb-4">Já existe um exame anexado a este registro.</p>
                      <div className="flex gap-2">
                        <a 
