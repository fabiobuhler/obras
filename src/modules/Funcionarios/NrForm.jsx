...
const nrSchema = z.object({
  nr: z.string().min(1, 'O número da NR é obrigatório'),
...
export default function NrForm({ initialData, funcionarioId, onSubmit, onCancel }) {
  const [selectedFile, setSelectedFile] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadMode, setUploadMode] = useState('file');

  const { register, handleSubmit, formState: { errors }, reset, setValue, watch } = useForm({
    resolver: zodResolver(nrSchema),
    defaultValues: initialData || { status: 'Válido' }
  });
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            {...register('nr')}
            className={`w-full rounded-md border bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary ${errors.nr ? 'border-red-500' : 'border-gray-300 dark:border-gray-700'}`}
            placeholder="Ex: NR-10, NR-35"
          />
          {errors.nr && <span className="text-red-500 text-xs">{errors.nr.message}</span>}
        </div>
        ...
        <div className="col-span-2">
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium">Anexar Certificado</label>
          ...
          {uploadMode === 'link' ? (
            <input
              ...
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              placeholder="https://meu-drive.com/nr35.pdf"
            />
          ) : (
                ...
                <>
                  ...
                  {arquivoUrlAtual && !selectedFile && (
                    <div className="absolute inset-0 bg-white dark:bg-zinc-900 flex flex-col items-center justify-center rounded-md z-10 p-4 border-2 border-green-500/50">
                      ...
                      <p className="text-sm text-center mb-4">Já existe um certificado anexado a esta NR.</p>
                      <div className="flex gap-2">
                        <a 
