...
const abastecimentoSchema = z.object({
  ...
  data_abastecimento: z.string().optional(),
  combustivel: z.string().optional(),
  litros: z.coerce.number().optional().nullable(),
  valor_total: z.number().optional().nullable(),
...
export default function AbastecimentoForm({ initialData, equipamentoId, onSubmit, onCancel }) {
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      const payload = {
        ...
        data_abastecimento: data.data_abastecimento || null,
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1">Data de Abastecimento</label>
          <input type="date" {...register('data_abastecimento')} className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Combustível</label>
          <input {...register('combustivel')} className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm" placeholder="Ex: Diesel S10" />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Posto / Fornecedor</label>
        ...
        <div>
          ...
          <input type="number" step="0.01" {...register('litros')} className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm" />
        </div>
        <div>
          ...
          <CurrencyInput
            name="valor_total"
            control={control}
            defaultValue={initialData?.valor_total}
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-2">
          <label className="block text-sm font-medium">Anexar Comprovante / Cupom Fiscal</label>
        ...
        {uploadMode === 'link' ? (
          <input {...register('arquivo_url')} className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm" placeholder="https://meu-drive.com/cupom.jpg" />
        ) : (
              ...
              <>
                ...
                {arquivoUrlAtual && !selectedFile && (
                  <div className="absolute inset-0 bg-white dark:bg-zinc-900 flex flex-col items-center justify-center rounded-md z-10 p-4 border-2 border-green-500/50">
                    ...
                    <p className="text-sm text-center mb-4">Já existe um comprovante anexado a este registro.</p>
                    <div className="flex gap-2">
